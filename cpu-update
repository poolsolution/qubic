...
if [ ${#token} -ge 61 ]; then
  echo "{\"Settings\":{\"baseUrl\": \"https://mine.qubic.li/\",\"amountOfThreads\": $threads,\"alias\": \"$minerAlias\",\"accessToken\": \"$token\",\"allowHwInfoCollect\": false,\"autoupdateEnabled\": true }}" > $path/$settingsFile;
else
  echo "{\"Settings\":{\"baseUrl\": \"https://mine.qubic.li/\",\"amountOfThreads\": $threads,\"alias\": \"$minerAlias\",\"accessToken\": null,\"payoutId\": \"$token\",\"allowHwInfoCollect\": false,\"autoupdateEnabled\": true }}" > $path/$settingsFile;
fi
